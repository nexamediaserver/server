...
public interface IMetadataItemRepository
{
    ...
    /// <summary>
    /// Retrieves cast members (person metadata) related to the provided metadata items.
    /// </summary>
    /// <param name="metadataIds">Database identifiers of the metadata items.</param>
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <returns>A mapping from metadata id to its cast members.</returns>
    Task<IReadOnlyDictionary<int, IReadOnlyList<CastMember>>> GetCastByMetadataIdsAsync(
