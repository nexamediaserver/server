...
public interface ILibrarySectionService
{
    ...
    /// </summary>
    /// <param name="uuid">The unique identifier of the library section to remove.</param>
    /// <returns>A task that represents the asynchronous operation. The task result contains true if the section was found and removed; otherwise, false.</returns>
    Task<bool> RemoveLibrarySectionAsync(Guid uuid);
}
