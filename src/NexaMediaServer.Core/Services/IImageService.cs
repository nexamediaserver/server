...
public interface IImageService
{
    ...
    /// <summary>
    /// Ingests external artwork referenced by a parser or metadata agent, saving it under the item's artwork folder.
    /// </summary>
    /// <param name="item">The metadata item.</param>
    /// <param name="sourceIdentifier">Identifier of the provider/parser/agent to use for naming and precedence.</param>
    /// <param name="kind">Artwork kind (poster, backdrop, logo).</param>
    /// <param name="source">Absolute file path or HTTP/HTTPS URL to fetch.</param>
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <returns>Canonical artwork URI (metadata://posters/{sourceIdentifier}_{uuid}) or null on failure.</returns>
    Task<string?> IngestExternalArtworkAsync(
        MetadataItem item,
    ...
    /// <summary>
    /// Computes a ThumbHash placeholder from the full-size source image.
    /// </summary>
    /// <param name="sourceUri">The metadata:// URI or absolute file path for the source image.</param>
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <returns>Base64-encoded ThumbHash or null if the source cannot be read.</returns>
    Task<string?> ComputeThumbHashAsync(string sourceUri, CancellationToken cancellationToken);
}
