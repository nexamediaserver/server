...
#pragma warning restore S6960
{
    ...
    )
    {
        try
        {
            ...
            // Generate ETag based on file path hash (stable for same output) and file size
            var fileInfo = new FileInfo(outputPath);
            var etagValue = GenerateETagFast(outputPath, fileInfo.Length);
    ...
    /// <summary>
    /// Generates a fast ETag using string hash code and file size (no crypto).
    /// The cache key in the path already incorporates content parameters.
    /// </summary>
    private static string GenerateETagFast(string path, long fileSize)
    {
        var pathHash = path.GetHashCode();
        return $"{pathHash:x8}{fileSize:x8}";
    }
}
