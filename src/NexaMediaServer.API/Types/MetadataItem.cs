...
public class MetadataItem
{
    ...
    /// <summary>
    /// Resolves extras related to the current metadata item.
    /// </summary>
    /// <param name="extrasLoader">Data loader for extras.</param>
    /// <param name="cancellationToken">Token used to cancel the resolver.</param>
    /// <returns>Extras associated with this item.</returns>
    [GraphQLName("extras")]
    public async Task<IReadOnlyList<MetadataItem>> GetExtrasAsync(
        IExtrasByMetadataIdDataLoader extrasLoader,
        CancellationToken cancellationToken
    )
    {
        ...
        var extras = await extrasLoader.LoadAsync(this.DatabaseId, cancellationToken);
        return extras ?? Array.Empty<MetadataItem>();
    }
