...
import { type AgentItemState, SortableAgentList } from './SortableAgentList'
...
const availableMetadataAgentsQuery = graphql(`
...
export function AddContentSourceDialog({
  onCreated,
}: Readonly<AddContentSourceDialogProps>) {
  ...
  const [availableAgents, setAvailableAgents] = useState<MetadataAgentInfo[]>(
    [],
  )
  ...
  const [fetchAgents, { loading: agentsLoading }] = useLazyQuery(
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }: { value: FormValues }) => {
      ...
      const settings: LibrarySectionSettingsInput = {
        disabledMetadataAgents: value.disabledMetadataAgents,
      ...
      try {
        const result = await addLibrarySection({ variables: { input } })
        if (!result.data?.addLibrarySection.librarySection) {
          return
        }
        ...
        // Keep the dialog open so the user can review errors
        return
      }
    },
  ...
  // Fetch available metadata agents when navigating to agents tab
  useEffect(() => {
    if (tab === 'agents' && formValues.contentType && !agentsInitialized) {
      void fetchAgents({
        variables: { libraryType: formValues.contentType },
      }).then((result) => {
        const agents = result.data?.availableMetadataAgents ?? []
        setAvailableAgents(agents)

        // If form hasn't been initialized with agent order yet, set defaults
        if (formValues.metadataAgentOrder.length === 0) {
          const sortedAgents = [...agents].sort(
            (a, b) => a.defaultOrder - b.defaultOrder,
          )
          form.setFieldValue(
            'metadataAgentOrder',
            sortedAgents.map((a) => a.name),
          )
        }
    ...
    formValues.metadataAgentOrder.length,
    agentsInitialized,
    fetchAgents,
    form,
  ])

  // Derive agent item states for the sortable list
  const agentItems: AgentItemState[] = formValues.metadataAgentOrder
    .map((name) => {
      ...
      return {
        agent,
        enabled: !formValues.disabledMetadataAgents.includes(name),
      }
    })
    .filter((item): item is AgentItemState => item !== null)

  const handleAgentOrderChange = useCallback(
    (newOrder: string[]) => {
      form.setFieldValue('metadataAgentOrder', newOrder)
    },
    [form],
  )
  ...
  const handleAgentToggle = useCallback(
    (agentName: string, enabled: boolean) => {
      const currentDisabled = formValues.disabledMetadataAgents
      if (enabled) {
        form.setFieldValue(
          'disabledMetadataAgents',
          currentDisabled.filter((name) => name !== agentName),
        )
      } else {
        form.setFieldValue('disabledMetadataAgents', [
          ...currentDisabled,
          agentName,
        ])
      }
    },
    [form, formValues.disabledMetadataAgents],
  )
  ...
  const nextStepKey =
    currentStepIndex < wizardSteps.length - 1
      ...
      : undefined
  const isFinalStep = tab === wizardSteps.at(-1)?.key

  let canContinue: boolean
  if (tab === 'category') {
    canContinue = hasCategorySelection
  } else if (tab === 'details') {
    canContinue = hasBasicsComplete
  } else if (tab === 'folders') {
    canContinue = hasDetailsComplete
  } else {
    canContinue = true
  }

  const disableSubmit = loading || !hasDetailsComplete
  let submitButtonLabel = 'Continue'
  if (isFinalStep) {
    if (loading) {
      submitButtonLabel = 'Creating…'
    } else {
      submitButtonLabel = 'Create library'
    }
  }
  ...
  function appendFolder(path: string) {
    ...
    if (formValues.folders.includes(trimmed)) {
      return false
    }
  ...
  const lastFolder =
    formValues.folders.length > 0 ? formValues.folders.at(-1) : null
          ...
          >
                    ...
                    <form.Field
                      ...
                      validators={{
                        onChange: ({ value }: { value: string }) =>
                          value.trim() ? undefined : 'Name required',
                      }}
                    >
                      {(field) => (
                        <div className="space-y-2">
                ...
                >
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="text-sm text-stone-300">
                        Drag agents to change the order they run. Toggle agents
                        off to skip them entirely. Higher agents take priority.
                      </p>
                    </div>
                    {agentsLoading && (
                    ...
                    {!agentsLoading && agentItems.length > 0 && (
                      <SortableAgentList
                        agents={agentItems}
                        onOrderChange={handleAgentOrderChange}
                    ...
                    {!agentsLoading && agentItems.length === 0 && (
                      <div
                        className={`
                          ...
                          py-8 text-center text-sm text-stone-400
                        `}
                      >
                        No metadata agents available for this library type.
                      </div>
                    )}
                  </div>
                </TabsContent>
            ...
            >
              ...
              <div className="flex">
                ...
                <Button
                  disabled={
                    isFinalStep ? disableSubmit : !canContinue || !nextStepKey
                  }
                  onClick={(event) => {
                    if (!isFinalStep && nextStepKey) {
                      event.preventDefault()
                      setTab(nextStepKey)
                    }
                  }}
