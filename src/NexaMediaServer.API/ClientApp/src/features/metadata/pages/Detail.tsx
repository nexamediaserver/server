...
const ContentSourceQuery = graphql(`
...
export function MetadataItemDetailPage(): ReactNode {
  ...
  const { data, error, loading } = useQuery(MetadataItemQuery, {
  ...
  const extras = data?.metadataItem?.extras ?? []

  if (!data?.metadataItem && !loading) {
    if (error) {
      console.error(error)

      return <div>Error loading metadata item.</div>
    }
