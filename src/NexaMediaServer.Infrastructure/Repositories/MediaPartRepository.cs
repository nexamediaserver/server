...
public class MediaPartRepository : IMediaPartRepository
{
    ...
    /// <inheritdoc />
    public Task<int> SoftDeleteByFilePathAsync(string part)
    {
        return this
            .context.MediaParts.IgnoreQueryFilters()
            .Where(mp => mp.File == part && mp.DeletedAt == null)
    ...
    )
    {
        ...
        var paths = await this.BuildFilePathQuery(libraryId).ToListAsync(cancellationToken);

        return paths.ToHashSet(StringComparer.OrdinalIgnoreCase);
    }

    /// <inheritdoc />
    public async Task<int> SoftDeleteByFilePathsAsync(
        ...
        foreach (var path in filePaths ?? Enumerable.Empty<string>())
        {
            ...
            if (pending.Count >= chunkSize)
            {
                totalDeleted += await this
                    .context.MediaParts.IgnoreQueryFilters()
                    .Where(mp => pending.Contains(mp.File) && mp.DeletedAt == null)
                    .ExecuteUpdateAsync(
                        s => s.SetProperty(p => p.DeletedAt, deletedAt),
                        cancellationToken
                    );
                pending.Clear();
            }
    ...
    )
    {
        ...
        return this.BuildFilePathQuery(libraryId).AsAsyncEnumerable();
    }

    private IQueryable<string> BuildFilePathQuery(int libraryId)
    {
        return this
