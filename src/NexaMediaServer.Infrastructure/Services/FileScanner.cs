...
public partial class FileScanner : IFileScanner
{
    ...
    )
    {
        ...
        while (stack.Count > 0)
        {
            ...
            if (this.ignoreRules.Any(r => r.ShouldIgnoreDirectory(current, parentDirectory)))
            {
                this.LogIgnoringDirectory(current);
            ...
            try
            {
                ...
                {
                    cancellationToken.ThrowIfCancellationRequested();
                    if (this.ignoreRules.Any(r => r.ShouldIgnoreFile(file, current)))
                    {
                        this.LogIgnoringFile(file);
                        continue;
                    }
    ...
    [LoggerMessage(Level = LogLevel.Debug, Message = "Ignoring directory: {Path}")]
    private partial void LogIgnoringDirectory(string path);

    [LoggerMessage(Level = LogLevel.Debug, Message = "Ignoring file: {Path}")]
    private partial void LogIgnoringFile(string path);
