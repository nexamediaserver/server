...
public sealed class PartsRegistry : IPartsRegistry
{
    ...
    private readonly ConcurrentBag<ISidecarParser> sidecarParsers = new();
    private readonly ConcurrentBag<IEmbeddedMetadataExtractor> embeddedMetadataExtractors = new();
    ...
    private ISidecarParser[] cachedSidecarParsers = Array.Empty<ISidecarParser>();
    private IEmbeddedMetadataExtractor[] cachedEmbeddedMetadataExtractors =
        Array.Empty<IEmbeddedMetadataExtractor>();
    ...
    /// <inheritdoc />
    public IReadOnlyList<ISidecarParser> SidecarParsers => this.cachedSidecarParsers;

    /// <inheritdoc />
    public IReadOnlyList<IEmbeddedMetadataExtractor> EmbeddedMetadataExtractors =>
        this.cachedEmbeddedMetadataExtractors;
    ...
    /// <summary>
    /// Gets the total count of registered sidecar parsers.
    /// </summary>
    internal int SidecarParserCount => this.cachedSidecarParsers.Length;

    /// <summary>
    /// Gets the total count of registered embedded metadata extractors.
    /// </summary>
    internal int EmbeddedMetadataExtractorCount => this.cachedEmbeddedMetadataExtractors.Length;
    ...
    /// <summary>
    /// Attempts to add a sidecar parser instance if not already present and registry not frozen.
    /// </summary>
    /// <param name="parser">The parser instance to add.</param>
    public void TryAddSidecarParser(ISidecarParser parser)
    {
        if (this.frozen || parser is null)
        {
            return;
        }

        if (this.sidecarParsers.Any(p => p.GetType() == parser.GetType()))
        {
            return;
        }

        this.sidecarParsers.Add(parser);
    }

    /// <summary>
    /// Attempts to add an embedded metadata extractor instance if not already present and registry not frozen.
    /// </summary>
    /// <param name="extractor">The extractor instance to add.</param>
    public void TryAddEmbeddedMetadataExtractor(IEmbeddedMetadataExtractor extractor)
    {
        if (this.frozen || extractor is null)
        {
            return;
        }

        if (this.embeddedMetadataExtractors.Any(e => e.GetType() == extractor.GetType()))
        {
            return;
        }

        this.embeddedMetadataExtractors.Add(extractor);
    }
    ...
    public void Freeze()
    {
        ...
        this.cachedMetadataAgents = this
            .metadataAgents.OrderBy(static agent => GetOrder(agent))
            .ToArray();
        this.cachedSidecarParsers = this
            .sidecarParsers.OrderBy(static parser => GetOrder(parser))
            .ToArray();
        this.cachedEmbeddedMetadataExtractors = this
            .embeddedMetadataExtractors.OrderBy(static extractor => GetOrder(extractor))
            .ToArray();
