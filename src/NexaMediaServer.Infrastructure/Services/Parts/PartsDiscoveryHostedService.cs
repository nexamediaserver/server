...
public sealed partial class PartsDiscoveryHostedService : IHostedService
{
    ...
    public Task StartAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            this.LogDiscoverySummary(
                ...
                this.registry.FileAnalyzerCount,
                this.registry.ImageProviderCount
            );
        }
        catch (Exception ex)
        {
    ...
    private void DiscoverFileAnalyzers(IEnumerable<Assembly> assemblies)
    {
        foreach (var type in assemblies.SelectMany(SafeGetTypes))
        {
            var analyzerInterfaces = GetGenericInterfaceImplementations(
                type,
                typeof(IFileAnalyzer<>)
            );
            if (
                analyzerInterfaces.Count == 0
                || type.IsAbstract
                || type.IsInterface
                || type.IsGenericTypeDefinition
            )
    ...
    private void DiscoverImageProviders(IEnumerable<Assembly> assemblies)
    {
        foreach (var type in assemblies.SelectMany(SafeGetTypes))
        {
            var providerInterfaces = GetGenericInterfaceImplementations(
                type,
                typeof(IImageProvider<>)
            );
            if (
                providerInterfaces.Count == 0
                || type.IsAbstract
                || type.IsInterface
                || type.IsGenericTypeDefinition
            )
