...
/// </summary>
public partial class GopIndexService : IGopIndexService
{
    ...
    /// <inheritdoc />
    public string GetGopPath(MetadataItem metadata, int partIndex)
    {
        ArgumentNullException.ThrowIfNull(metadata);
        var uuid = metadata.Uuid.ToString("N");
        if (uuid.Length < 2)
        {
            throw new ArgumentException("Invalid UUID for metadata.", nameof(metadata));
        }
    ...
    )
    {
        ...
        var gopPath = this.GetGopPath(metadata, partIndex);
    ...
    )
    {
        ...
        return this.TryReadAsync(metadata, partIndex, cancellationToken);
    }
