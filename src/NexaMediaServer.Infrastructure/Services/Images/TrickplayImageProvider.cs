...
/// </summary>
public partial class TrickplayImageProvider : IImageProvider
{
    ...
    /// <inheritdoc />
    public int Order => 200; // Run after thumbnail generation
    ...
    )
    {
        ...
        try
        {
            ...
            // Read extracted frames and build BIF file
            var frameFiles = IODirectory.GetFiles(tempDir, "frame_*.jpg").OrderBy(f => f).ToList();

            this.LogBatchExtractionComplete(item.Id, frameFiles.Count);

            for (int i = 0; i < frameFiles.Count && i < targetTimestamps.Count; i++)
            {
                cancellationToken.ThrowIfCancellationRequested();

                try
                {
                    var imageBytes = await File.ReadAllBytesAsync(frameFiles[i], cancellationToken)
                        .ConfigureAwait(false);
                    var timestampKey = (int)targetTimestamps[i];
                    bifFile.Entries.Add(new BifEntry(timestampKey, 0));
                    bifFile.ImageData[timestampKey] = imageBytes;

                    if ((i + 1) % 100 == 0)
                    {
                        this.LogBatchProgress(item.Id, i + 1, frameFiles.Count);
                    }
                }
                catch (Exception ex)
                {
    ...
    )
    {
        ...
        {
            var bifPath = this.bifService.GetBifPath(metadata, partIndex);
        ...
        {
            await this
                .bifService.WriteAsync(metadata, partIndex, bifFile, cancellationToken)
                .ConfigureAwait(false);
            var bifPath = this.bifService.GetBifPath(metadata, partIndex);
    ...
    [LoggerMessage(
        Level = LogLevel.Debug,
        Message = "Trickplay provider start: mediaItem={MediaItemId} metadata={MetadataId} parts={PartCount}"
    )]
