...
public sealed partial class MetadataService : IMetadataService
{
    ...
    public async Task GenerateImagesAsync(Guid metadataItemUuid)
    {
        ...
        var processingTasks = mediaItems.Select(async media =>
        {
            ...
            foreach (var provider in imageProviders)
            {
                ...
                try
                {
                    ...
                    var provideTask = provider.ProvideAsync(media, parts, CancellationToken.None);
        ...
        // Image providers update the DTO instance; sync any newly discovered thumbnail onto the tracked entity.
        if (
            !string.IsNullOrWhiteSpace(metadataDto.ThumbUri)
            && !string.Equals(meta.ThumbUri, metadataDto.ThumbUri, StringComparison.Ordinal)
        )
        {
            meta.ThumbUri = metadataDto.ThumbUri;
        }
        ...
        try
        {
            ...
            orderedAgentIdentifiers.AddRange(imageProviders.Select(p => p.Name));
