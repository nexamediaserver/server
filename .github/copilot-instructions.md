...
- Library scans (`LibraryScannerService`) stream `FileScanner` batches through bounded `Channel`s and bulk insert via `EFCore.BulkExtensions`; extend this pipeline instead of re-reading the filesystem.
...
- Backend uses .NET SDK 9.0.110 (`global.json`); `dotnet build NexaMediaServer.sln` and `dotnet test tests/NexaMediaServer.Tests.Unit/NexaMediaServer.Tests.Unit.csproj` are the baseline commands.
